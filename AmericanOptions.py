...
class AmericanCallOption:
    ...
    def generateOptionValueTree(self, market_model):
        ...
        for el_numb in range(1,market_model.number_of_layers + 1):
            intrinsic_value = self.value(market_model.getTreeNodeValue(market_model.number_of_layers, el_numb))
            continuation_value = 0

            if intrinsic_value > continuation_value:
                option_value_tree[market_model.getTreeId(market_model.number_of_layers,el_numb)] = intrinsic_value
                exercise_tree[market_model.getTreeId(market_model.number_of_layers,el_numb)] = 1  # Early exercise chosen
            else:
                option_value_tree[market_model.getTreeId(market_model.number_of_layers,el_numb)] = continuation_value
                exercise_tree[market_model.getTreeId(market_model.number_of_layers,el_numb)] = 0  # Continuation value chosen
